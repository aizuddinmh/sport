...
public $day_closed					= array("Sunday");
...
public $cost_currency_tag			= "RM";		// The currency tag in HTML such as &euro; &pound; &yen;
...
/*========================================================================================================================================================*/
	...
	function make_booking_array($year, $month, $j = 0) { 
		...
		$query = "SELECT * FROM bookings WHERE date LIKE '$year-$month%' AND c_id = '".$_SESSION['cid']."'"; 
	...
	function make_cells($table = '') {
		...
		foreach($this->days as $i => $r) { // Loop through the date array
			...
			if(isset($this->bookings_per_day[$current_day]) && $tag == 0) {
				...
				$current_day_slots_booked = count($this->bookings_per_day[$current_day]);
					...
					if($current_day_slots_booked < $this->slots_per_day) {
						...
						<a href='calendar.php?act=get&amp;month=" .  $this->month . "&amp;year=" .  $this->year . "&amp;day=" . sprintf("%02s", $r['daynumber']) . "' class='part_booked' title='This day is part booked'>" . 
						$r['daynumber'] . "</a></td>"; 
						$tag = 1;
					
					} else {
					
						echo "\r\n<td width='21' valign='top' class='fully_booked' title='This day is fully booked'>" . 
						$r['daynumber'] . "</a></td>"; 
						$tag = 1;			
			...
			if($tag == 0) {
				...
				echo "\r\n<td width='21' valign='top'>
				<a href='calendar.php?act=get&amp;month=" .  $this->month . "&amp;year=" .  $this->year . "&amp;day=" . sprintf("%02s", $r['daynumber']) . "' class='green' title='Please click to view bookings'>" . 
				$r['daynumber'] . "</a></td>";			
