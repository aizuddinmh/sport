...
this.DatePicker = Picker.Date = new Class({
	...
	getInputDate: function(input){
		...
		if (!input) return;
                //bye 31/01/2013
                dateValue = input.get('value').substring(3,5)+'/'+input.get('value').substring(0,2)+input.get('value').substring(5);    
	...
	days: function(days, options, currentDate, dateElements, fn){
		...
		if (weeknumbers) new Element('th.title.day.weeknumber', {
			text: Locale.get('DatePicker.week')
                ...
                //new Element('td colspan=7').inject(titles);
		for (day = options.startDay; day < (options.startDay + 7); day++){
			new Element('td.title.day.day' + (day % 7), {
		...
		days.each(function(_date, i){
			...
			element = new Element('td' + classes, {text: date.getDate(), role: 'gridcell'}).inject(weekcontainer);
                        //element = new Element('td', {text: date.getDate()}).inject(weekcontainer);
