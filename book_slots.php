...
if(isset($_POST['uid'])) $uid = mysqli_real_escape_string($link, $_POST['uid']);
if(isset($_POST['fid'])) $fid = mysqli_real_escape_string($link, $_POST['fid']);
...
foreach($explode as $slot) {
	...
	if(strlen($slot) > 0) {
		...
		$query = "INSERT INTO bookings (date, start, u_id, f_id) VALUES ('$booking_date', '$slot', '$uid', '$fid')"; 
